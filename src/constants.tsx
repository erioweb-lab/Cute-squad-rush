...
export const BASE_PLAYER_APS = 1.0;         // 기본 초당 공격 횟수 (Base Attacks Per Second)
export const FIRE_RATE = Math.floor(50 / BASE_PLAYER_APS); // 기본 발사 간격 (프레임 단위, 낮을수록 빠름)
...
export const FIRING_CIRCLE_SCALE = 0.45;    // 발사 시 시각적 효과 원의 크기 배율

// Game Balance Modifiers (게임 밸런스 조정 관련 상수)
export const DRONE_BASE_FIRE_RATE = 60; // 드론 기본 발사 간격 (프레임 단위)
export const DRONE_ATTACK_SPEED_MULTIPLIER = 1.0; // 드론 공격 속도 배율 (1.0 = 기본, 0.5 = 50% 감소)
export const PLAYER_ATTACK_SPEED_MULTIPLIER = 1.0; // 캐릭터 공격 속도 배율 (1.0 = 기본, 1.3 = 30% 증가)
export const BASE_PLAYER_DAMAGE = 5.0;     // 기본 공격력 (Base Damage)
export const PLAYER_DAMAGE_MULTIPLIER = 1.0; // 캐릭터 데미지 배율 (1.0 = 기본, 0.8 = 20% 감소)
export const PLAYER_MIN_DAMAGE_HP_EQUIVALENT = 1; // 체력이 낮아져도 유지되는 최소 데미지 기준 HP

// Character-specific Growth Constants (캐릭터별 성장 상수)
export interface CharacterLevelStats {
  damage: number;       // 추가 데미지
  aps: number;          // 추가 공격 속도 (0.1 = 10%)
  critChance: number;   // 추가 치명타 확률 (%)
  critDamage: number;   // 추가 치명타 데미지 배율 (0.05 = 5%)
  pierce: number;       // 추가 관통 확률 (0.1 = 10%)
}
...
export interface CharacterGrowth {
  ...
  basePierceBonus: number;
  upgradeStats: CharacterLevelStats[]; // 0, 1, 2, 3, 4 단계별 스탯
}

export const CHARACTER_GROWTH: Record<string, CharacterGrowth> = {
  cat: {
    damagePerHp: 0,       // HP당 데미지 증가 (현재 0)
    baseApsBonus: 0.1,    // 기본 공격 속도 보너스 (10%)
    baseDamageBonus: 0,   // 기본 데미지 보너스
    baseCritBonus: 0,     // 기본 치명타 확률 보너스
    basePierceBonus: 0,   // 기본 관통 확률 보너스
    upgradeStats: [
      // { damage: 추가 데미지, aps: 추가 공속 보너스, critChance: 추가 치명타 확률(%), critDamage: 추가 치명타 피해 배율, pierce: 추가 관통 확률 }
      { damage: 0, aps: 0.1, critChance: 0, critDamage: 0, pierce: 0.02 },
      { damage: 1, aps: 0.2, critChance: 2, critDamage: 0.01, pierce: 0.04 },
      { damage: 2, aps: 0.3, critChance: 4, critDamage: 0.02, pierce: 0.06 },
      { damage: 3, aps: 0.4, critChance: 6, critDamage: 0.03, pierce: 0.08 },
      { damage: 4, aps: 0.5, critChance: 8, critDamage: 0.04, pierce: 0.10 }
    ]
  },
  rabbit: {
    damagePerHp: 0,       // HP당 데미지 증가
    baseApsBonus: 0,      // 기본 공격 속도 보너스
    baseDamageBonus: 0,   // 기본 데미지 보너스
    baseCritBonus: 0,     // 기본 치명타 확률 보너스
    basePierceBonus: 0.1, // 기본 관통 확률 보너스 (10%)
    upgradeStats: [
      { damage: 0, aps: 0, critChance: 0, critDamage: 0, pierce: 0.1 },
      { damage: 1, aps: 0.05, critChance: 2, critDamage: 0.01, pierce: 0.15 },
      { damage: 2, aps: 0.1, critChance: 4, critDamage: 0.02, pierce: 0.2 },
      { damage: 3, aps: 0.15, critChance: 6, critDamage: 0.03, pierce: 0.25 },
      { damage: 4, aps: 0.2, critChance: 8, critDamage: 0.04, pierce: 0.3 }
    ]
  },
  rat: {
    damagePerHp: 0,       // HP당 데미지 증가
    baseApsBonus: 0,      // 기본 공격 속도 보너스
    baseDamageBonus: 0,   // 기본 데미지 보너스
    baseCritBonus: 10,    // 기본 치명타 확률 보너스 (10%)
    basePierceBonus: 0,   // 기본 관통 확률 보너스
    upgradeStats: [
      { damage: 0, aps: 0, critChance: 10, critDamage: 0, pierce: 0 },
      { damage: 1, aps: 0.05, critChance: 10, critDamage: 0.05, pierce: 0.02 },
      { damage: 2, aps: 0.1, critChance: 10, critDamage: 0.1, pierce: 0.04 },
      { damage: 3, aps: 0.15, critChance: 10, critDamage: 0.15, pierce: 0.06 },
      { damage: 4, aps: 0.2, critChance: 10, critDamage: 0.2, pierce: 0.08 }
    ]
  },
  dog: {
    damagePerHp: 0,       // HP당 데미지 증가
    baseApsBonus: 0,      // 기본 공격 속도 보너스
    baseDamageBonus: 0.1, // 기본 공격력 보너스 (10%)
    baseCritBonus: 0,     // 기본 치명타 확률 보너스
    basePierceBonus: 0,   // 기본 관통 확률 보너스
    upgradeStats: [
      { damage: 1, aps: 0, critChance: 0, critDamage: 0, pierce: 0 },
      { damage: 2, aps: 0.03, critChance: 2, critDamage: 0.01, pierce: 0.03 },
      { damage: 4, aps: 0.06, critChance: 4, critDamage: 0.02, pierce: 0.06 },
      { damage: 6, aps: 0.09, critChance: 6, critDamage: 0.03, pierce: 0.09 },
      { damage: 8, aps: 0.12, critChance: 8, critDamage: 0.04, pierce: 0.12 }
    ]
  }
};

export const RAT_CRIT_DAMAGE_BONUS = 0.05; // 쥐 치명타 데미지 보너스 (5%)

export const PIERCE_BASE_PROBABILITY = 0.0; // 관통 공격 시 기본 관통 확률 (0%)
export const PIERCE_PROBABILITY_PER_LEVEL = 0.05; // 관통 단계별 추가 확률 (5%)
...
export const BOSS_MINION_SPAWN_MULTIPLIER = 0.6; // 보스 슬라임(미니언) 스폰 숫자 배율 (0.5 = 50% 감소)
export const BULLET_POWER_MULTIPLIER = 1.0; // 캐릭터 탄환 공격력 배율
...
export const MAX_BULLETS = 500; // 최대 탄환 수 (성능 최적화)
export const MAX_PARTICLES = 300; // 최대 파티클 수
export const MAX_ENEMIES = 150; // 최대 적 수
...
export const CHAR_ICE_DURATION = 30; // 빙결 상태이상 지속 시간 (프레임)
export const CHAR_ELECTRIC_STUN_DURATION = 10; // 전기 기절 상태이상 지속 시간 (프레임, 0.5초)
...
export const ROCK_CRACKER_DAMAGE = 10;  // Rock Cracker 돌 공격력
...
export const BOSS_FIRING_CONFIG = {
  ...
  TOXIC_CLOUD:    { bulletCount: 0.3, bulletSpeed: 1.5 },
  ACID_STALKER:   { bulletCount: 0.3, bulletSpeed: 1.5 },
  VOLT_SPIKER:    { bulletCount: 1.2, bulletSpeed: 1.1 },
  ROCK_CRACKER:   { bulletCount: 0.5, bulletSpeed: 0.8 },
...
export const createParticles = (state: GameState, x: number, y: number, color: string, count: number) => {
  ...
  const actualCount = Math.min(count, MAX_PARTICLES - state.particles.length);
...
export const SKILL_PROPS = {
  // 관통 확률 (%): 1레벨당 증가하는 관통 확률 (예: 5 = 5%)
  pierce: { chance: 5, cost: 100, scaling: 1.5, maxLevel: 4 },
  // 멀티샷 (개): 1레벨당 추가되는 발사체 수
  multiShot: { bullets: 1, cost: 100, scaling: 1.8, maxLevel: 3 },
  // 공격 속도 (%): 1레벨당 증가하는 공격 속도 (예: 2 = 2%)
  attackSpeed: { speed: 2, cost: 100, scaling: 1.6, maxLevel: 10 },
  // 데미지 증가 (%): 1레벨당 증가하는 데미지 (예: 5 = 5%)
  damageUp: { damage: 5, cost: 100, scaling: 1.6, maxLevel: 10 },
  // 총알 크기 (%): 1레벨당 증가하는 총알 크기 (예: 2 = 2%)
  bulletSize: { size: 2, cost: 100, scaling: 1.5, maxLevel: 5 },
  // 최대 체력 (고정값): 1레벨당 증가하는 최대 체력
  maxHp: { hp: 20, cost: 200, scaling: 1.4, maxLevel: 5 },
  // 치명타 확률 (%): 1레벨당 증가하는 치명타 확률 (예: 3 = 3%)
  critChance: { chance: 3, cost: 100, scaling: 1.7, maxLevel: 5 },
  // 자석 범위 (%): 1레벨당 증가하는 아이템 획득 범위 (예: 5 = 5%)
  magnetRange: { range: 5, cost: 100, scaling: 1.3, maxLevel: 5 },
  // 피버 지속시간 (초): 1레벨당 증가하는 피버 지속시간
  feverDuration: { duration: 1, cost: 100, scaling: 1.5, maxLevel: 5 },
  // 쉴드 획득량 (고정값): 1레벨당 추가로 획득하는 쉴드량
  shieldDuration: { shield: 1, cost: 100, scaling: 1.5, maxLevel: 5 },
  // 점수 획득량 (%): 1레벨당 증가하는 점수 획득량 (예: 10 = 10%)
  scoreMult: { score: 10, cost: 100, scaling: 2.0, maxLevel: 3 },
  // 흡혈 (%): 1레벨당 데미지 비례 체력 회복량 (예: 5 = 5%)
  vampiricBullets: { heal: 5, cost: 150, scaling: 1.8, maxLevel: 3 },
  // 폭발 확률 (%): 1레벨당 폭발 총알 발동 확률 (예: 5 = 5%)
  explosiveBullets: { chance: 5, cost: 150, scaling: 1.8, maxLevel: 3 },
  // 이동 속도 (%): 1레벨당 증가하는 이동 속도 (예: 5 = 5%)
  speedySquad: { speed: 5, cost: 100, scaling: 1.4, maxLevel: 5 },
};

export const UPGRADE_CONFIG: Record<string, UpgradeConfig> = {
  pierce: { name: 'Piercing Bullets', description: `+${SKILL_PROPS.pierce.chance}% Pierce Chance`, icon: '🏹', baseCost: SKILL_PROPS.pierce.cost, costScaling: SKILL_PROPS.pierce.scaling, maxLevel: SKILL_PROPS.pierce.maxLevel, levelPerValue: `+${SKILL_PROPS.pierce.chance}% Pierce Chance` },
  multiShot: { name: 'Multi-Shot', description: `+${SKILL_PROPS.multiShot.bullets} Bullet`, icon: '🎯', baseCost: SKILL_PROPS.multiShot.cost, costScaling: SKILL_PROPS.multiShot.scaling, maxLevel: SKILL_PROPS.multiShot.maxLevel, levelPerValue: `+${SKILL_PROPS.multiShot.bullets} Bullet` },
  attackSpeed: { name: 'Attack Speed', description: `+${SKILL_PROPS.attackSpeed.speed}% Attack Speed`, icon: '⚡', baseCost: SKILL_PROPS.attackSpeed.cost, costScaling: SKILL_PROPS.attackSpeed.scaling, maxLevel: SKILL_PROPS.attackSpeed.maxLevel, levelPerValue: `+${SKILL_PROPS.attackSpeed.speed}%` },
  damageUp: { name: 'Damage Up', description: `+${SKILL_PROPS.damageUp.damage}% Damage`, icon: '💥', baseCost: SKILL_PROPS.damageUp.cost, costScaling: SKILL_PROPS.damageUp.scaling, maxLevel: SKILL_PROPS.damageUp.maxLevel, levelPerValue: `+${SKILL_PROPS.damageUp.damage}%` },
  bulletSize: { name: 'Giant Bullets', description: `+${SKILL_PROPS.bulletSize.size}% Bullet Size`, icon: '🔮', baseCost: SKILL_PROPS.bulletSize.cost, costScaling: SKILL_PROPS.bulletSize.scaling, maxLevel: SKILL_PROPS.bulletSize.maxLevel, levelPerValue: `+${SKILL_PROPS.bulletSize.size}%` },
  maxHp: { name: 'Max HP Up', description: `+${SKILL_PROPS.maxHp.hp} Max HP`, icon: '❤️', baseCost: SKILL_PROPS.maxHp.cost, costScaling: SKILL_PROPS.maxHp.scaling, maxLevel: SKILL_PROPS.maxHp.maxLevel, levelPerValue: `+${SKILL_PROPS.maxHp.hp} HP` },
  critChance: { name: 'Critical Eye', description: `+${SKILL_PROPS.critChance.chance}% Crit Chance`, icon: '👁️', baseCost: SKILL_PROPS.critChance.cost, costScaling: SKILL_PROPS.critChance.scaling, maxLevel: SKILL_PROPS.critChance.maxLevel, levelPerValue: `+${SKILL_PROPS.critChance.chance}%` },
  magnetRange: { name: 'Magnetism', description: `+${SKILL_PROPS.magnetRange.range}% Magnet Range`, icon: '🧲', baseCost: SKILL_PROPS.magnetRange.cost, costScaling: SKILL_PROPS.magnetRange.scaling, maxLevel: SKILL_PROPS.magnetRange.maxLevel, levelPerValue: `+${SKILL_PROPS.magnetRange.range}%` },
  feverDuration: { name: 'Fever Master', description: `+${SKILL_PROPS.feverDuration.duration}s Fever Duration`, icon: '🔥', baseCost: SKILL_PROPS.feverDuration.cost, costScaling: SKILL_PROPS.feverDuration.scaling, maxLevel: SKILL_PROPS.feverDuration.maxLevel, levelPerValue: `+${SKILL_PROPS.feverDuration.duration}s` },
  shieldDuration: { name: 'Shield Master', description: `+${SKILL_PROPS.shieldDuration.shield} Shield per pickup`, icon: '🛡️', baseCost: SKILL_PROPS.shieldDuration.cost, costScaling: SKILL_PROPS.shieldDuration.scaling, maxLevel: SKILL_PROPS.shieldDuration.maxLevel, levelPerValue: `+${SKILL_PROPS.shieldDuration.shield} Shield` },
  scoreMult: { name: 'Gold Rush', description: `+${SKILL_PROPS.scoreMult.score}% Score Gain`, icon: '💰', baseCost: SKILL_PROPS.scoreMult.cost, costScaling: SKILL_PROPS.scoreMult.scaling, maxLevel: SKILL_PROPS.scoreMult.maxLevel, levelPerValue: `+${SKILL_PROPS.scoreMult.score}%` },
  vampiricBullets: { name: 'Vampiric Bullets', description: `Heal ${SKILL_PROPS.vampiricBullets.heal}% of damage`, icon: '🧛', baseCost: SKILL_PROPS.vampiricBullets.cost, costScaling: SKILL_PROPS.vampiricBullets.scaling, maxLevel: SKILL_PROPS.vampiricBullets.maxLevel, levelPerValue: `+${SKILL_PROPS.vampiricBullets.heal}%` },
  explosiveBullets: { name: 'Explosive Bullets', description: `${SKILL_PROPS.explosiveBullets.chance}% Explosion Chance`, icon: '🧨', baseCost: SKILL_PROPS.explosiveBullets.cost, costScaling: SKILL_PROPS.explosiveBullets.scaling, maxLevel: SKILL_PROPS.explosiveBullets.maxLevel, levelPerValue: `+${SKILL_PROPS.explosiveBullets.chance}%` },
  speedySquad: { name: 'Speedy Squad', description: `+${SKILL_PROPS.speedySquad.speed}% Movement Speed`, icon: '🏃', baseCost: SKILL_PROPS.speedySquad.cost, costScaling: SKILL_PROPS.speedySquad.scaling, maxLevel: SKILL_PROPS.speedySquad.maxLevel, levelPerValue: `+${SKILL_PROPS.speedySquad.speed}%` },
};

export const AVAILABLE_SKILLS: RogueliteSkill[] = [
  { id: 'pierce', name: UPGRADE_CONFIG.pierce.name, description: `Bullets have a ${SKILL_PROPS.pierce.chance}% chance to pierce through enemies.`, icon: UPGRADE_CONFIG.pierce.icon, rarity: 'COMMON', value: 1 },
  { id: 'multiShot', name: UPGRADE_CONFIG.multiShot.name, description: `Fire +${SKILL_PROPS.multiShot.bullets} extra bullet per shot.`, icon: UPGRADE_CONFIG.multiShot.icon, rarity: 'RARE', value: SKILL_PROPS.multiShot.bullets },
  { id: 'attackSpeed', name: UPGRADE_CONFIG.attackSpeed.name, description: `Increase attack speed by ${SKILL_PROPS.attackSpeed.speed}%.`, icon: UPGRADE_CONFIG.attackSpeed.icon, rarity: 'COMMON', value: 1 },
  { id: 'damageUp', name: UPGRADE_CONFIG.damageUp.name, description: `Increase damage by ${SKILL_PROPS.damageUp.damage}%.`, icon: UPGRADE_CONFIG.damageUp.icon, rarity: 'COMMON', value: 1 },
  { id: 'bulletSize', name: UPGRADE_CONFIG.bulletSize.name, description: `Increase bullet size and hitbox by ${SKILL_PROPS.bulletSize.size}%.`, icon: UPGRADE_CONFIG.bulletSize.icon, rarity: 'COMMON', value: 1 },
  { id: 'maxHp', name: UPGRADE_CONFIG.maxHp.name, description: `Increase max HP by ${SKILL_PROPS.maxHp.hp} and heal.`, icon: UPGRADE_CONFIG.maxHp.icon, rarity: 'RARE', value: SKILL_PROPS.maxHp.hp },
  { id: 'heal', name: 'Squad Heal', description: 'Restore 20 HP to all players.', icon: '🩹', rarity: 'COMMON', value: 20 },
  { id: 'critChance', name: UPGRADE_CONFIG.critChance.name, description: `Increase critical hit chance by ${SKILL_PROPS.critChance.chance}%.`, icon: UPGRADE_CONFIG.critChance.icon, rarity: 'RARE', value: 1 },
  { id: 'shieldStart', name: 'Shield Starter', description: 'Start each stage with +1 shield.', icon: '🛡️', rarity: 'EPIC', value: 1 },
  { id: 'magnetRange', name: UPGRADE_CONFIG.magnetRange.name, description: `Increase magnet range by ${SKILL_PROPS.magnetRange.range}%.`, icon: UPGRADE_CONFIG.magnetRange.icon, rarity: 'COMMON', value: 1 },
  ...
  { id: 'damageUp', name: 'God Strength', description: `Increase damage by ${SKILL_PROPS.damageUp.damage * 3}%.`, icon: '🔱', rarity: 'LEGENDARY', value: 3 },
  { id: 'feverDuration', name: UPGRADE_CONFIG.feverDuration.name, description: `Increase fever duration by ${SKILL_PROPS.feverDuration.duration * 2} seconds.`, icon: UPGRADE_CONFIG.feverDuration.icon, rarity: 'RARE', value: 1 },
  { id: 'shieldDuration', name: UPGRADE_CONFIG.shieldDuration.name, description: `Get +${SKILL_PROPS.shieldDuration.shield} extra shield per pickup.`, icon: UPGRADE_CONFIG.shieldDuration.icon, rarity: 'RARE', value: SKILL_PROPS.shieldDuration.shield },
  { id: 'scoreMult', name: UPGRADE_CONFIG.scoreMult.name, description: `Increase score gain by ${SKILL_PROPS.scoreMult.score}%.`, icon: UPGRADE_CONFIG.scoreMult.icon, rarity: 'LEGENDARY', value: 1 },
  { id: 'vampiricBullets', name: UPGRADE_CONFIG.vampiricBullets.name, description: `Heal ${SKILL_PROPS.vampiricBullets.heal}% of damage dealt.`, icon: UPGRADE_CONFIG.vampiricBullets.icon, rarity: 'EPIC', value: SKILL_PROPS.vampiricBullets.heal },
  { id: 'explosiveBullets', name: UPGRADE_CONFIG.explosiveBullets.name, description: `${SKILL_PROPS.explosiveBullets.chance}% chance to explode on hit.`, icon: UPGRADE_CONFIG.explosiveBullets.icon, rarity: 'EPIC', value: 1 },
  { id: 'speedySquad', name: UPGRADE_CONFIG.speedySquad.name, description: `Increase movement speed by ${SKILL_PROPS.speedySquad.speed}%.`, icon: UPGRADE_CONFIG.speedySquad.icon, rarity: 'RARE', value: 1 },
];
...
export type GameState = {
  ...
  bullets: Array<{ x: number; y: number; vx?: number; vy?: number; damage: number; id: number; types: string[]; isCrit: boolean; rabbitPierce?: boolean; pierceCount: number; hitEnemies: number[]; hitGates?: number[]; size: number; playerStage?: number; character?: string }>;
...
export const playSound = (type: 'shoot' | 'hit' | 'explosion' | 'powerup' | 'boss' | 'fever') => {
  ...
  // Throttle frequent sounds to prevent audio engine overload and freezing
